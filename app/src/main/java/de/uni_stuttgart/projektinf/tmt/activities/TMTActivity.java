...
public class TMTActivity extends AppCompatActivity {
    ...
    private Circle findPivotCircle(Circle lastInInnerLayer, List<Circle> innerLayerList, List<Circle> outerLayerList) {
        ...
        for(Circle testPivot : outerLayerList) {
            ...
            // then test the new segment in the outer layer (which will be created by rearranging
            // the circles in the outer layer with the pivot) against the inner layer circles:
            Circle newSeg1 = outerLayerList.get(outerLayerList.size()-1);
            Circle newSeg2 = outerLayerList.get(0);
            for (int j = 0; j < innerLayerList.size() - 1; j++) {
                Circle circ1 = innerLayerList.get(j);
                Circle circ2 = innerLayerList.get(j + 1);
                if (testSegmentsIntersect(newSeg1, newSeg2, circ1, circ2))
                    continue testPivotLoop;
            }
            // then test the new segment in the outer layer (which will be created by rearranging
            // the circles in the outer layer with the pivot) against the other outer layer circles:
            for (int j = 0; j < outerLayerList.size() - 1; j++) {
                Circle circ1 = outerLayerList.get(j);
                Circle circ2 = outerLayerList.get(j + 1);
                // skip checkings containing the new segment itself:
                if (!circ1.equals(newSeg1))
                    if (!circ1.equals(newSeg2))
                        if (!circ2.equals(newSeg1))
                            if (!circ2.equals(newSeg2))
                                if (testSegmentsIntersect(newSeg1, newSeg2, circ1, circ2))
                                    continue testPivotLoop;
            }
